...
#
# Not scripts/run-eval.sh, on purpose: that script hard-overrides the database
...
#
# Usage: scripts/run-flow-eval.sh <go-test-run-pattern>
#   scripts/run-flow-eval.sh TestEvalIndex              # build the corpus
#   scripts/run-flow-eval.sh TestExpandFlowQuestions    # freeze the expansions
#   scripts/run-flow-eval.sh TestFlowGathered           # what reaches the answer
#   scripts/run-flow-eval.sh 'TestFlowEdgeReach$'       # the edge walk on its own
#   scripts/run-flow-eval.sh 'TestEvalMeasureAnswers$'  # the answers, judged
#
...
#   BACKEND_EVAL_GAP=1 scripts/run-flow-eval.sh 'TestEvalMeasureAnswers$'
...
cd backend
# -count=1 for the reason scripts/run-eval.sh gives: these arms read state Go
