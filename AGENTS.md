...
- **A project turn is not a comparison of its own repositories.** `Scope.Projects` lists projects `Known` covers ENTIRELY; one suppresses the comparison rule, two or more compare by project name. A partial cover counts for nothing.
...
### Repos and index
- **A structure edit is not a re-index.** Changing `project`/`kind`/`description`/`uses` leaves `last_sha`, `branch` and the checkout alone; the reset trigger stays `clone_url`.
- **`repos.Load` refuses**: a missing `project`; a `project` equal to a NON-member repository's name (one project per member name is the normal single-repo case); a `uses` entry unknown, cross-project or self; one `clone_url` twice in a project.
