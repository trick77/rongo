...
- **German is Swiss by string function, never prompt** (`backend/internal/ask/swiss.go`). Static German strings written `ss` by hand. Code never touched: fences, inline spans, identifier-shaped words. A prompt note was measured useless (`2026-09-16-swiss-digraphs.md`) — do not add one back. `TestEvalMeasureAnswers` prints `digraphs N [words]`; read the words.
