...
- **Those two modules are one unit.** The vec extension is a wasm build compiled against ncruces' SQLite, so version skew breaks vector search instead of the build. Bump both or neither, never a lone Dependabot PR. Same pair as peeq and loom (`v0.23.3` / `v0.1.7-alpha.2`) so fixes transfer; the alpha needs its changelog read first.
...
- **Every claim is citable**: repo, branch, file, line. Cited files are never evicted when capping context.
