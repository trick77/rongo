...
- **A property crossing needs a reason.** Code reading `${key}` always crosses. Config to config only on a key sharing a word with the question: an infra hit crossed on `spring.*` and `sentry.*` into every service and took 22.9k of 24k tokens. Property landings together spend at most the crossing reserve.
