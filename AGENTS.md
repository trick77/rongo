...
- **Data is filtered, docs are not** (`2026-09-17-data-file-cap.md`). Path-and-size rules re-applied at boot, so a new rule retires what an older build embedded. Body-only verdicts (secret, marker) are not. **xsd/wsdl are contracts, not data**: own 256 KiB ceiling, above it code tables; read by `ExtractXSD`, never ctags. Schema kinds anchor only in a schema file: `message`/`service`/`element` are Protobuf/SQL/DTD ctags kinds too.
...
- **A project turn is not a comparison of its own repositories**, in the prompt AND the search: one fused search over the members at `comparisonK`, never one per member merged by raw score (per-search RRF ranks put 19 of 40 hits in an off-topic infra repo). Partial cover counts for nothing: a leftover repository drops the turn to repository-grained comparison.
...
- **Never gather outside the turn's projects.** Projects of what was named, else of what the search hit, plus the libraries they declare in `uses`. Walk, crossings, gap pass and locate loop all admit through that one ceiling. A live turn named one project and gathered from 11 repositories.
...
- **Repo list in `repos.yaml`, credentials never** — that file reaches a repo or ticket eventually. Tokens from `BACKEND_*`, injected at fetch time, never logged. Repos page is read-only status, no CRUD form. **Its one action is a full re-index** — per repository, per project (its members), or all — admin only, confirmed with the cost said honestly (every file re-read, only changed content re-embedded; "all" says so louder). It is a request generation on the row that the poller honours at its next pass and clears only for the generation it read, so a click mid-run is served by the next cycle and a failed run leaves the index as it was; the poller is nudged so the cycle starts now.
...
- **A property crossing needs a reason.** Code reading `${key}` always crosses. Config to config only on a key sharing a word with the turn's search texts (question, previous question, terms, code terms): an infra hit crossed on `spring.*` and `sentry.*` into every service and took 22.9k of 24k tokens. Config-to-config landings together spend at most the crossing reserve. No words: no filter, no cap.
- **A new extraction rule needs a forced re-index.**
- **vec0 never frees a deleted vector** — the storage chunk stays, inserts only append, and every KNN reads every chunk's blob. 46k vectors in 380 chunks made each search take minutes; turns died as `chunks iter error`, which is sqlite-vec's word for an interrupt. `CompactVectors` rebuilds past 2× AT BOOT ONLY: it holds the write lock ~10 s per 14k vectors, past `busy_timeout`, so beside live turns it fails their writes. Index runs warn.
