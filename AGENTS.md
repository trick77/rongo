...
- **Clarify only on real ambiguity.** Candidates depending on each other per `repo_deps` are composition, not alternatives — answer all of them. A question naming ≥2 indexed repos is a comparison: never a card (rung in `ask.Decide`, before margin), one search per named repo, answer covers every one.
