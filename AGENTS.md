...
- **Code is the source of truth; docs add context.** README, docs and comments carry intent the code alone does not show, so they stay indexed — but the code decides what happens. Contradiction → the answer says so and sides with the code. Docs written for reading, not for the corpus (plans, mock-ups), go into `BACKEND_INDEX_EXCLUDE`, never a broad doc exclusion.
